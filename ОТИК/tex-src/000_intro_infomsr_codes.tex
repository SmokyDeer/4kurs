...
\begin{document}
\author[\AlmaMaterAbbrev, \MyUnitAbbrev, \MyAbbrev]{\mbox{\MyIofFull} / {\bfseries illinc\MVAt mail.ru} \mbox{+7-985-148-32-64 (телефон), +7-977-977-97-29 (WhatsApp),}  \url{gitlab.com/illinc/raspisanie}}
...
.
\end{enumerate}
% В 2020 г. посещаемость лекций не оценивается баллами ввиду их~(лекций) дистанционности. 
\smallskip

Экзамен (оценка):\\\hfill
\begin{tabular}{ll}
\termin{$5$} ~~ $86-100$\\
\termin{$4$} ~~ $70-85$\\
\termin{$3$} ~~ $50-69$\\
...
\item {стационарный} (вероятность символа не зависит от времени/позиции: только от контекста) / {нестационарный} (при сдвиге  вероятности меняются);

% \item случайная последовательность, где вероятность следующего символа определяется только его предысторией;

\item \termin{марковский} источник "---вероятность символа определяется состоянием; 
...
% $I(x)$
% должна быть монотонно связана с~затратами на передачу сообщения $x \in X$.
% \vspace{-1em}
\begin{enumerate}
\item $I(x) \geqslant 0$.
...
% $|X| = N$, вероятности $x \in X$ равны:
Источник $X$ порождает $N$ \termin{равновероятных} сообщений $x$ ($\forall x \in X:~ p(x) =p = \frac{1}{N}$).
\vspace{-0.5\baselineskip}

$$ I(x) = I(X) = I = \log_2 N = - \log_2 (p) ~~~~\text{или $2^I = N$}$$
где
...
Свойства:%\vspace{-1em}
\begin{enumerate}
\item Неотрицательность: $I(x) \geqslant 0, x \in X$.
\item Монотонность: \rlap{$x_1, x_2 \in X, p(x_1) \geqslant p(x_2) \to I(x_1) \leqslant I(x_2)$.}
\item Аддитивность: для независимых сообщений $x_1, \ldots, x_n$
$I(x_1, \ldots, x_n) = \sum\limits_{i=1}^{n}I(x_i)$
...
{
$
I(X) = \sum\limits_{x_i\in X} \Big( p(x_i)\cdot I(x_i) \Big) = - \sum\limits_{x_i\in X} \Big( p(x_i)\cdot \log_2 p(x_i)\Big)
$\centering
...
$I(\vec x) = I(x_1) + \ldots + I(x_i| x_{i-m} \ldots x_{i-1})  + \ldots+ I(x_k| x_{k-1} \ldots x_{k-1})$
